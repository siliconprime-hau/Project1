...
//nvhau--
unsigned int
	SOIL_load_OGL_texture_ex_from_file
	(
		FILE *file,
		int force_channels,
		unsigned int reuse_texture_ID,
	...
	)
{
	...
	//}
	/*	try to load the image	*/
	img = SOIL_load_image_from_file( file, &width, &height, &channels, force_channels );
	/*	channels holds the original number of channels, which may have been forced	*/
	if( (force_channels >= 1) && (force_channels <= 4) )
	{
		channels = force_channels;
...
unsigned int
	SOIL_load_OGL_texture_ex_from_memory
	(
		const unsigned char *const buffer,
		int buffer_length,
		int force_channels,
...
{
	return SOIL_load_OGL_texture_ex(filename, force_channels, reuse_texture_ID, flags, NULL, NULL);
}
...
{
	unsigned char *result = stbi_load_from_file( file,
			width, height, channels, force_channels );
	if( result == NULL )
	{
		result_string_pointer = stbi_failure_reason();
